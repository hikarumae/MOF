        ...
        <main className="max-w-5xl mx-auto px-6 py-20 flex flex-col items-center gap-16">
          ...
          <section className="w-full max-w-3xl">
            ...
            className={`cursor-pointer border-2 border-dashed rounded-xl p-10 flex flex-col items-center justify-center gap-6 text-center h-56 transition-all ${isDragging 
              ...
              {/* 画面には見えないが、ファイル選択ダイアログを開くためのinput。ドラッグ中に文字を変更　*/}
              <input type="file" ref={fileInputRef} onChange={handleFileChange} className="hidden" />
                <p className="font-bold text-slate-700">
                  {isDragging ? "ここにドロップしてアップロード" : "ファイル保存"}
                </p>

                <button className="bg-slate-500 hover:bg-slate-600 text-white px-10 py-3 rounded-md shadow-md font-medium transition-colors w-64 pointer-events-none">ファイルを保存する
                </button>
            </div>
          </section>
          ...
          <div className="p-8 max-w-4xl mx-auto space-y-8">
            ...
            <div className="space-y-6">
              ...
              <section className="w-full max-w-3xl">
                ...
                onDragOver={handleDragOver} 
                onDragLeave={handleDragLeave} 
                onDrop={handleDrop} className={`cursor-pointer border-2 border-dashed rounded-xl p-10 flex flex-col items-center justify-center gap-6 text-center h-56 transition-all ${isDragging 
              ? "border-blue-700 bg-blue-100" // ドラッグ中の強調スタイル
              : "border-blue-500 bg-blue-50/20 hover:bg-blue-50/40" // 通常時のスタイル
              }`}>
